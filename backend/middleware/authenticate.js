...
async function authenticate(req, res, next) {
  try {
    ...
    if (!token) {
      if (process.env.NODE_ENV !== 'production') console.log('[DEBUG] authenticate: No token provided');
    ...
    try {
      ...
      if (process.env.NODE_ENV !== 'production') console.log('[DEBUG] authenticate: Token verification failed:', err.name, err.message);
    ...
    if (!decoded?.id) {
      if (process.env.NODE_ENV !== 'production') console.log('[DEBUG] authenticate: Malformed token (no id)');
